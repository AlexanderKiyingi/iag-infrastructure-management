...
// settings (Kafka topics, upstream clients) are added as the infrastructure
// domain is implemented; this skeleton wires only the platform plumbing.
type Config struct {
	Environment string
	ServiceName string
	Port        string
...
func Load() (Config, error) {
	...
	c := Config{
		Environment:         env,
		ServiceName:         getenv("SERVICE_NAME", "infrastructure-management"),
		Port:                getenv("PORT", "4104"),
		...
		Audience:            getenv("AUDIENCE", "iag.infrastructure-management"),
		ServiceClientID:     getenv("SERVICE_CLIENT_ID", "iag-infrastructure-management"),
