...
import (
	...
	"iag-infrastructure-management/backend/internal/db"
	"iag-infrastructure-management/backend/internal/middleware"
...
func NewRouter(deps RouterDeps) *gin.Engine {
	...
	}
	{
		...
		// Domain endpoints (facilities, utilities, capex, maintenance) are
		// added here as the infrastructure-management domain is implemented.
		// The skeleton exposes a permission-gated overview so RBAC wiring is
		// exercised.
		v1.GET("/overview", middleware.RequirePermission("infrastructure.view_overview"), func(c *gin.Context) {
			c.JSON(http.StatusOK, gin.H{
				"service": deps.Cfg.ServiceName,
				"status":  "scaffold",
				"message": "infrastructure-management domain not yet implemented",
			})
		})
	}
	return r
